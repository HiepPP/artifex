...
the Vietnamese gate. `scripts/bundle.sh` writes `dist/Rustelier.app`.
...
filesystem watcher, no persistence, no drag reorder, no editor selection or find
bar, no image diffs, no Mermaid. The POC is not aiming at feature parity.
